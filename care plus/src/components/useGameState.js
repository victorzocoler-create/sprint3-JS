...
const DEFAULT_STATE = {
  ...
  pet: DEFAULT_PET,
  // Poderes
  unlockedPowers: ['fire'],   // começa com fogo desbloqueado para demo
  battleHistory: {},           // { opponentId: { lastBattle, wins, losses } }
  totalCheckups: 1,
  exerciseStreak: 7,
  waterStreak: 14,
  meditationDays: 21,
...
export function useGameState() {
  ...
  const toggleChallenge = useCallback((challengeId) => {
    ...
    setState(prev => {
      ...
      if (done) { delete newCompleted[challengeId] }
      else {
  ...
  const redeemVoucher = useCallback((voucher) => {
    setState(prev => {
      if (prev.ps < voucher.ps) { showToast(`PS insuficientes!`); return prev }
      if (prev.redeemedVouchers.includes(voucher.id)) { showToast('Já resgatado!'); return prev }
      if (!voucher.available) { showToast('Disponível apenas no nível Guardião+'); return prev }
      spawnParticles()
      showToast(`✓ "${voucher.title}" resgatado! −${voucher.ps.toLocaleString('pt-BR')} PS`)
      return { ...prev, ps: prev.ps - voucher.ps, redeemedVouchers: [...prev.redeemedVouchers, voucher.id] }
    })
  }, [showToast, spawnParticles])

  const carePet = useCallback((action) => {
    setState(prev => {
      if (prev.ps < action.cost) { showToast(`PS insuficientes!`); return prev }
      spawnParticles()
      showToast(`${action.label} feito! −${action.cost} PS`)
      return {
        ...prev,
        ps: prev.ps - action.cost,
        pet: { ...prev.pet, [action.stat]: Math.min(100, prev.pet[action.stat] + action.boost) }
      }
    })
  }, [showToast, spawnParticles])

  // Batalha — retorna resultado rodada a rodada
  const doBattle = useCallback((opponent, playerMoves) => {
    // playerMoves = array de 3 power ids escolhidos pelo jogador
    ...
    setState(prev => {
      ...
      spawnParticles()
      showToast(playerWins
        ? `Vitória! +200 PS ganhos! 🏆`
        : `Derrota! −150 PS perdidos 😢`
      )
      return {
        ...prev,
  ...
  return {
    ...
    toggleChallenge, redeemVoucher, carePet, doBattle, showToast,
  }
}